...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  ...
  const description = job.description_snippet ? `${job.description_snippet.slice(0, 155)}...` : `${job.title} internship at ${job.company_name} in ${location}.`;
  return {
    title,
    description,
    openGraph: { title, description, url: `${BASE_URL}/internships/${job.id}`, type: "article" },
  ...
  return (
    <div style={{ maxWidth: 860, margin: "0 auto", padding: "40px 24px" }}>
      <Link href="/internships" style={{
        ...
        fontSize: 13, color: "var(--text-3)", textDecoration: "none", marginBottom: 28,
      }}>
      ...
      <div className="card" style={{ borderLeft: `4px solid ${cc.color}`, padding: "clamp(24px, 5vw, 40px)", marginBottom: 16 }}>
        ...
        <h1 style={{
          ...
          fontWeight: 700, color: "var(--text)", margin: "0 0 10px", lineHeight: 1.2,
        }}>{job.title}</h1>
        ...
        <div className="detail-meta-grid">
          {[
            ...
            <div key={m.label} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid var(--border)", borderRadius: 10, padding: "14px 16px" }}>
              ...
              <p style={{ fontWeight: 600, color: m.green ? "#4ade80" : "var(--text)", fontSize: 13, margin: 0 }}>{m.value}</p>
            </div>
          ))}
        </div>
        ...
        <div className="detail-actions">
          <a href={job.apply_url} target="_blank" rel="noopener noreferrer" className="btn-gold" style={{ fontSize: 14, padding: "12px 24px" }}>
            Apply on {SOURCE_LABELS[job.source] || job.source} <ExternalLink size={13} />
          </a>
          <Link href={`/alerts?country=${encodeURIComponent(job.location_country)}&industry=${encodeURIComponent(job.industry || "")}`} className="btn-ghost" style={{ fontSize: 14 }}>
